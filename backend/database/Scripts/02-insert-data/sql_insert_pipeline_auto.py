...
class SQLPipeline:
    def __init__(self):
        ...
        self.standard_sql_files = [
            '01-INSERT-CATEGORIAS.sql',
            '02-INSERT-MARCAS.sql',
            '03-INSERT-TIPO-ENVIO.sql',
            '04-INSERT-ESTADO-ORDEN.sql',
            '05-INSERT-METODO-PAGO.sql'
        ]
        # Bulk load SQL files (from Bulk-Load directory)
        self.bulk_load_sql_files = [
            '06-INSERT-USUARIOS.sql',
            '07-INSERT-DIRECCIONES.sql',
            '08-INSERT-USUARIO-METODO-PAGO.sql',
            '09-INSERT-PRODUCTOS.sql',
            '10-INSERT-PRODUCTO-CATEGORIA.sql',
            '11-INSERT-IMAGENES.sql',
            '12-INSERT-CARRITOS.sql',
            '13-INSERT-CARRITO-ITEMS.sql',
            '14-INSERT-ORDENES.sql',
            '15-INSERT-ORDEN-ITEMS.sql',
            '16-INSERT-PAGOS.sql',
            '17-INSERT-RESENAS.sql',
            '18-INSERT-AUDITORIA.sql'
        ]
    ...
    def parse_arguments(self):
        ...
        parser.add_argument('--db-name', default='ecommerce_db',
                          help='Database name (default: ecommerce_db)')
        parser.add_argument('--schema-name', default='ecommerce',
                          help='Schema name (default: ecommerce)')
    ...
    def find_sql_directory(self) -> Optional[str]:
        ...
        return self.args.sql_dir  # Return the provided dir even if we couldn't confidently verify it
    ...
    def run(self):
        ...
        try:
            ...
            if os.path.exists(bulk_load_dir):
                ...
                logger.warning(f"Bulk-Load directory not found at {bulk_load_dir}. Expected for bulk logic.")
            ...
            with tqdm(all_files, desc="Processing SQL files") as pbar_files:
                for sql_file_name, file_path in pbar_files:
                    ...
                    if os.path.exists(file_path):
                        ...
                        logger.warning(f"File {file_path} omitted because it has not been created yet.")
...
def get_project_root():
    """Returns the absolute path to the data directory assumption"""
