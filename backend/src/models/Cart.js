...
const addItem = async (userId, productId, quantity = 1) => {
  ...
  try {
    ...
    const cartId = cartRes.rows[0].id_carrito;

    const prodRes = await client.query(
      'SELECT precio FROM ecommerce.producto WHERE id_producto = $1',
      [productId]
    );
  ...
  await db.query(
    `DELETE FROM ecommerce.carrito_item
     WHERE id_carrito_item = $1
       AND id_carrito IN (
         SELECT id_carrito FROM ecommerce.carrito WHERE id_usuario = $2
       )`,
    [itemId, userId]
  );
};
